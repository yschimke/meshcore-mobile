...
/**
 * Quick check whether a BLE device is nearby before attempting a full connection. Used by
 * background workers to avoid wasting battery on connections that will time out.
 *
 * For TCP/USB devices, always returns true (no proximity check possible).
 */
object DeviceProximityCheck {

  /**
   * Returns true if the device is likely reachable:
   * - TCP/USB devices: always true
  ...
  suspend fun isNearby(context: Context, device: SavedDevice, scanTimeoutMs: Long = 5_000): Boolean {
    ...
    val found =
      withTimeoutOrNull(scanTimeoutMs) {
        scanner.advertisements.firstOrNull { adv -> adv.identifier == ble.identifier }
      }
