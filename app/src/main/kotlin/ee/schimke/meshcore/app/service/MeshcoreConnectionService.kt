 ...
 *
 * - [ee.schimke.meshcore.app.connection.AppConnectionController] starts
 *   this service on connect and stops it on disconnect.
...
class MeshcoreConnectionService : Service() {
    ...
    /**
     * Delegates to the debug-only [DebugBridge] when one is installed.
     * Release builds have no provider registering the bridge, so this
     * prints a short placeholder instead of exposing any introspection.
     */
    override fun dump(fd: FileDescriptor, writer: PrintWriter, args: Array<String>?) {
        val bridge = DebugBridge.instance
        if (bridge == null) {
            writer.println("MeshcoreConnectionService: debug surface not compiled into this build")
            return
        }
        try {
            bridge.dumpService(this, writer, args)
        } catch (t: Throwable) {
            writer.println("dump error: ${t::class.simpleName}: ${t.message}")
        } finally {
            writer.flush()
        }
    }
