...
fun Modifier.verticalScrollbar(
  ...
  drawWithContent {
    ...
    val itemHeight = visibleItems.map { it.size.toFloat() }.average().toFloat().coerceAtLeast(1f)
...
private fun DrawScope.drawRoundedThumb(color: Color, topLeft: Offset, size: Size, radius: Float) {
  drawRoundRect(
    color = color,
    topLeft = topLeft,
    size = size,
