...
/**
 * Format a chat message timestamp for display (`HH:mm` today, `dd/MM HH:mm` otherwise).
 * Platform-specific because date/time formatting is JVM-only here; Android and desktop both back it
 * with `java.time`.
 */
internal expect fun formatChatTimestamp(instant: Instant): String
    ...
    LazyColumn(
      ...
      items(reversed, key = { it.id }) { msg ->
        ChatBubble(
          ...
          onRetry =
            if (msg.status == MessageStatus.Failed && onRetry != null) {
              { onRetry(msg) }
            } else null,
        )
      }
...
fun ChatBubble(message: ChatMessage, modifier: Modifier = Modifier, onRetry: (() -> Unit)? = null) {
  ...
  Box(
    ...
    Surface(shape = shape, color = bubbleColor, modifier = Modifier) {
      Column(modifier = Modifier.padding(horizontal = 12.dp, vertical = 8.dp)) {
        ...
        Text(
          ...
          modifier =
            Modifier.padding(
              top = if (!message.isMine && message.senderName != null) 2.dp else 0.dp
            ),
        )
        ...
        Row(
          ...
          if (message.isMine) {
            Text(text = "·", style = MaterialTheme.typography.labelSmall, color = subtleColor)
            AnimatedContent(
              ...
              Text(
                ...
                style = MaterialTheme.typography.labelSmall,
                color =
                  if (status == MessageStatus.Failed) MaterialTheme.colorScheme.error
                  else subtleColor,
                modifier =
